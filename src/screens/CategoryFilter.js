...
const CategoryFilter = ({ navigation, route }) => {
    ...
    const filteredRestaurants = restaurants.filter((restaurant) => {
        return restaurant.restaurant.categorie.type === selectedCategory;
    });
    useEffect(() => {
        (async () => {
    ...
    return (
        <View>
           <Picker
                ...
                style={{ height: 50, width: '100%', backgroundColor: COLORS.white }}
                onValueChange={(itemValue, itemIndex) => handleCategoryChange(itemValue)}
            >
                <Picker.Item label="All" value="" />
                {Array.from(new Set(restaurants.map((r) => r.restaurant.categorie.type))).map((categories) => (
